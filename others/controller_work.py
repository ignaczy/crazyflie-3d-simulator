...
class BaseController:
    """Wspólny interfejs dla wszystkich algorytmów sterowania (Wzorzec Strategii)."""
    def compute(self, target: float, current: float, dt: float, v_ff: float = 0.0) -> float:
...
class PIDController(BaseController):
    ...
    def compute(self, target, current, dt, v_ff=0.0):
        if dt <= 0:
            return 0.0
        ...
        # Dodanie członu feedforward (v_ff) bezpośrednio do wyjścia prędkościowego
        out = self.Kp * error + self.Ki * self.integral + self.Kd * derivative + v_ff
...
class ADRCController(BaseController):
    """
    Liniowy regulator ADRC 2. rzędu z 3-stanowym obserwatorem ESO 
    oraz stabilizacją numeryczną metodą podkroków (sub-stepping).
    """
    ...
    def compute(self, target, current, dt, v_ff=0.0):
        ...
        # Uwzględnienie feedforward prędkości w prawie sterowania ADRC
        u_unclipped = (self.kp * error - self.kd * self.z2 - self.z3) / self.b0 + v_ff
...
class MPCController:
    def __init__(self, horizon=10, Q_pos=10.0, R_u=1.0, limit=1.0):
        self.N = int(horizon)          # Horyzont predykcji
        self.Q = float(Q_pos)          # Waga błędu pozycji
        self.R = float(R_u)            # Waga wysiłku sterowania
        self.limit = float(limit)      # Limit prędkości wyjściowej
        self.last_val = 0.0
        self.reset()
    ...
    def compute(self, setpoint, pv, dt, v_ff=0.0):
        """
        Pełnoprawne MPC z modelem podwójnego integratoratora i optymalizacją analityczną.
        """
        if dt <= 0:
            dt = 0.016
        ...
        R_ref = np.full(self.N, setpoint, dtype=float)
        ...
        try:
            ...
            # Bazowe sterowanie z MPC + dodanie składowej feedforward prędkości
            v_des = U_opt[0] + v_ff
