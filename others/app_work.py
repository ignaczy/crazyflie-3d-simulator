...
def render_logger_overlay(surface, texture_id, x, y, w, h, screen_w, screen_h, font_bold, font_small, logger_panel, logger_obj):
    ...
    glDisable(GL_LIGHTING)  # Naprawia przyciemnienie/ciemny wygląd tekstury nakładki
...
class SimulatorApp:
    def __init__(self):
        ...
        pygame.display.set_caption("Crazyflie 2.1+ | Zaawansowany Symulator Sterowania Kaskadowego")
        ...
        self.btn_reset = Button(panel_x, 285, slider_w, 24, "RESET STANU (SPACE)")
    ...
    def create_controllers(self, algo_type):
        if algo_type == "ADRC":
            return (
                ADRCController(b0=10.0, wc=15.0, wo=30.0, limit=1.0),
                ADRCController(b0=10.0, wc=15.0, wo=30.0, limit=1.0),
                ADRCController(b0=5.0,  wc=10.0, wo=25.0, limit=1.0)
            )
        elif algo_type == "MPC":
            return (
                MPCController(horizon=10, Q_pos=10.0, R_u=1.0, limit=1.0),
                MPCController(horizon=10, Q_pos=10.0, R_u=1.0, limit=1.0),
                MPCController(horizon=10, Q_pos=15.0, R_u=1.0, limit=1.0)
            )
        else:
            return (
    ...
    def handle_algorithm_switching(self):
        if self.algo_panel.current_algo != self.current_algo:
            ...
            if self.current_algo == "ADRC":
                ...
                self.slider_1.label, self.slider_1.min_val, self.slider_1.max_val, self.slider_1.val = "Horyzont N (X/Y)", 2.0, 30.0, float(self.pid_x.N)
                self.slider_2.label, self.slider_2.min_val, self.slider_2.max_val, self.slider_2.val = "Waga Q (X/Y)", 1.0, 50.0, self.pid_x.Q
                self.slider_3.label, self.slider_3.min_val, self.slider_3.max_val, self.slider_3.val = "Waga R (X/Y)", 0.1, 10.0, self.pid_x.R
                self.slider_4.label, self.slider_4.min_val, self.slider_4.max_val, self.slider_4.val = "Horyzont N (Z)", 2.0, 30.0, float(self.pid_z.N)
                self.slider_5.label, self.slider_5.min_val, self.slider_5.max_val, self.slider_5.val = "Waga Q (Z)", 1.0, 50.0, self.pid_z.Q
                self.slider_6.label, self.slider_6.min_val, self.slider_6.max_val, self.slider_6.val = "Waga R (Z)", 0.1, 10.0, self.pid_z.R
            else:
    ...
    def update_simulation(self, dt, current_ticks):
        ...
        if dt > 0:
            ...
            self.drone.step(target_pos, 0.0, dt, self.pids, v_ff=self.traj_gen.vel)
        ...
        if dt > 0:
            self.logger.sample(current_ticks / 1000.0, SystemWrapper(self))
    ...
    def render(self, current_ticks):
        ...
        draw_chart(self.gui_surface, pygame.Rect(panel_x, self.chart_y_starts[4], self.chart_w, self.chart_h), self.font_small,
                    "Sygnały sterujące Vx, Vy, Vz [m/s]", -1.2, 1.2,
                    [
